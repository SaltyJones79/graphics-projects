...
After that the ghost, and bow and arrow are rendered. The program is the first
step in making a halloween game that lets the user shoot the ghost and hockey masks. 
*/
...
var arrowPosition = vec2(0.0, -5.5); // Initial position at the bow
var arrowStartPosition = vec2(0.0, -5.5); // Initial position at the bow
var arrowEndPosition;
var arrowCurrentPosition = vec2(0.0, -5.5);
var arrowIsFiring = false;
var arrowSpeed = 0.2; // Control the speed of the arrow
...
function DrawHockeyMask(scaleFactor, position, rotationAngle) {
    ...
    let ovalScalingMatrix = scale4(.75, 2, 1);
...
function fireArrow() {
    if (!arrowIsFiring) {
      ...
      var travelDistance = 20; // Adjust as necessary for your canvas size
  
      // Calculate the arrow's end position
      arrowEndPosition = vec2(
        arrowStartPosition[0] + Math.cos(angleInRadians) * travelDistance,
        arrowStartPosition[1] + Math.sin(angleInRadians) * travelDistance
      );
    }
  }
  ...
  function animateArrow() {
    if (arrowIsFiring) {
      // Interpolate the arrow's current position towards the end position
      arrowCurrentPosition[0] += (arrowEndPosition[0] - arrowCurrentPosition[0]) * arrowSpeed;
      arrowCurrentPosition[1] += (arrowEndPosition[1] - arrowCurrentPosition[1]) * arrowSpeed;
  
      // Check if the arrow has reached near the end position to stop firing
      if (Math.abs(arrowCurrentPosition[0] - arrowEndPosition[0]) < 0.1 &&
          Math.abs(arrowCurrentPosition[1] - arrowEndPosition[1]) < 0.1) {
        arrowIsFiring = false;
        ghostState = 0; // Make the ghost disappear if necessary
        // Reset arrow position to the bow
        arrowCurrentPosition = vec2(arrowStartPosition[0], arrowStartPosition[1]);
      }
    }
  }
...
function render() {
    ...
    DrawObject(0.90, vec2(2.68, -2.8), 30);
    DrawObject2(0.85, vec2(2.5, -5), 0);
    DrawObject(0.90, vec2(-1.75, -3.75), 30);
    ...
    DrawHockeyMask(0.4, vec2(1.25, -2.50), 0.0);
    DrawHockeyMask(0.3, vec2(-3.0, -3.30), 0.0);
    ...
    DrawBowAndString(0.75, vec2(0.0, -5.5), 0);

    modelViewMatrix = mat4();  // Reset the modelViewMatrix
    gl.uniformMatrix4fv(modelViewMatrixLoc, false, flatten(modelViewMatrix));
    if (arrowState === 'ready') {
        DrawArrow(0.75, vec2(0.0, -5.5), 0);
    } else if (arrowState === 'firing') {
        animateArrow();
        DrawArrow(0.75, arrowPosition, 0); // Use the current rotation angle of your bow
    }
